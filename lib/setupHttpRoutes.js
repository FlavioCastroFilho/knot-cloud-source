...
function setupHttpRoutes(server, skynet){
  ...
  server.delete('/mydevices', function(req, res){
    ...
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, query, false, function(error, data){
        if(data.error){
        ...
        for(var i = dataGet.devices.length - 1; i >= 0; i--){
          skynet.sendActivity(getActivity('unregister', req, fromDevice));
          unregister(fromDevice, dataGet.devices[i].uuid, req.merged_params.token, skynet.emitToClient, function(err, data){
            if(err){
              meshbluEventEmitter.emit('unregister-error', {request: {uuid: data.uuid}, fromUuid: fromDevice.uuid, error: err});
              return errorResponse(err, res);
            }

            meshbluEventEmitter.emit('unregister', {request: {uuid: data.uuid}, fromUuid: fromDevice.uuid});
            output.push(data.uuid);
            if(output.length === dataGet.devices.length)
              if(dataGet.devices.length === 1){
