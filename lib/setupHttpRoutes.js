      ...
      getDevices(fromDevice, query, false, function(error, data){
        ...
        for(var i = length - 1; i >= 0; i--){
          ...
          unregister(fromDevice, dataGet.devices[i].uuid, req.merged_params.token, skynet.emitToClient, function(err, data){
            ...
            if(err){
              meshbluEventEmitter.emit('unregister-error', {request: {uuid: dataGet.devices[ii].uuid}, fromUuid: fromDevice.uuid, error: err});
              return errorResponse(err, res);
            }

            meshbluEventEmitter.emit('unregister', {request: {uuid: dataGet.devices[ii].uuid}, fromUuid: fromDevice.uuid});
            output = output.concat(data.uuid);
